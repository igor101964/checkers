...
void find_captures(int row, int col, Move* moves, int* count, int depth, Move* base);
...
void find_captures(int row, int col, Move* moves, int* count, int depth, Move* base) {
    ...
    for (int dir = 0; dir < 4; dir++) {
        ...
        if (is_king) {
            /* Дамка: сканируем диагональ до вражеской фигуры */
            int scan_r = row + dr, scan_c = col + dc;
            while (is_valid_square(scan_r, scan_c) &&
                   get_piece_at(scan_r, scan_c) == EMPTY) {
                scan_r += dr; scan_c += dc;
            }
            if (!is_valid_square(scan_r, scan_c)) continue;
            int sp = get_piece_at(scan_r, scan_c);
            if (sp == EMPTY) continue;
            int sp_pl = (sp==WHITE_MAN||sp==WHITE_KING)?WHITE:BLACK;
            if (sp_pl == player) continue;
            /* Проверяем что эта фигура ещё не съедена в текущей серии */
            if (base) {
                int already = 0;
                for (int k=0; k<base->num_captures; k++)
                    if (base->captured_pieces[k][0]==scan_r &&
                        base->captured_pieces[k][1]==scan_c) { already=1; break; }
                if (already) continue;
            }
            int jump_r = scan_r, jump_c = scan_c;
            /* Приземляемся на все клетки за вражеской */
            int land_r = jump_r + dr, land_c = jump_c + dc;
            while (is_valid_square(land_r, land_c) &&
                   get_piece_at(land_r, land_c) == EMPTY) {
                if (*count >= MAX_LEGAL_MOVES) break;
                /* Строим ход */
                Move nm;
                if (base) {
                    nm = *base;
                } else {
                    nm.from_row = row; nm.from_col = col;
                    nm.num_captures = 0;
                }
                nm.to_row = land_r; nm.to_col = land_c;
                nm.captured_pieces[nm.num_captures][0] = jump_r;
                nm.captured_pieces[nm.num_captures][1] = jump_c;
                nm.num_captures++;

                /* Временно применяем */
                int oj = get_piece_at(jump_r, jump_c);
                int ol = get_piece_at(land_r, land_c);
                int os = get_piece_at(row, col);
                set_piece_at(jump_r, jump_c, EMPTY);
                set_piece_at(land_r, land_c, piece);
                set_piece_at(row,    col,    EMPTY);

                int cb = *count;
                find_captures(land_r, land_c, moves, count, depth+1, &nm);

                set_piece_at(jump_r, jump_c, oj);
                set_piece_at(land_r, land_c, ol);
                set_piece_at(row,    col,    os);

                if (*count == cb) {
                    /* Нет продолжения — записываем */
                    get_move_notation(&nm);
                    moves[*count] = nm;
                    (*count)++;
                }
                land_r += dr; land_c += dc;
            ...
            /* Простая шашка: один прыжок */
            int jump_r = row + dr, jump_c = col + dc;
            int land_r = row + 2*dr, land_c = col + 2*dc;
            if (!is_valid_square(land_r, land_c)) continue;
            int jp = get_piece_at(jump_r, jump_c);
            if (jp == EMPTY) continue;
            if (get_piece_at(land_r, land_c) != EMPTY) continue;
            int jp_pl = (jp==WHITE_MAN||jp==WHITE_KING)?WHITE:BLACK;
            if (jp_pl == player) continue;
            /* Проверяем что эта фигура ещё не съедена */
            if (base) {
                int already = 0;
                for (int k=0; k<base->num_captures; k++)
                    if (base->captured_pieces[k][0]==jump_r &&
                        base->captured_pieces[k][1]==jump_c) { already=1; break; }
                if (already) continue;
            }
            /* Строим ход */
            Move nm;
...
void find_regular_moves(int row, int col, Move* moves, int* count) {
    ...
    int directions[4][2] = {{-1, -1}, {-1, 1}, {1, -1}, {1, 1}};
    /* Простая шашка: только вперёд. Дамка: все направления, любое расстояние */
    int start_dir = 0, end_dir = 4;
    if (!is_king && player == WHITE) end_dir = 2;
    else if (!is_king && player == BLACK) start_dir = 2;
    
    for (int dir = start_dir; dir < end_dir; dir++) {
        int dr = directions[dir][0];
        int dc = directions[dir][1];
        if (is_king) {
            /* Дамка летит по диагонали до препятствия */
            int nr = row + dr, nc = col + dc;
            while (is_valid_square(nr, nc) && get_piece_at(nr, nc) == EMPTY) {
                if (*count >= MAX_LEGAL_MOVES) break;
                Move move;
                move.from_row = row; move.from_col = col;
                move.to_row = nr;   move.to_col = nc;
                move.num_captures = 0;
                get_move_notation(&move);
                moves[*count] = move;
                (*count)++;
                nr += dr; nc += dc;
            }
        } else {
            /* Простая шашка — один шаг */
            int new_row = row + dr, new_col = col + dc;
            if (is_valid_square(new_row, new_col) && get_piece_at(new_row, new_col) == EMPTY) {
                Move move;
                move.from_row = row; move.from_col = col;
                move.to_row = new_row; move.to_col = new_col;
                move.num_captures = 0;
                moves[*count] = move;
                (*count)++;
            }
...
void generate_legal_moves(void) {
    ...
    if (game.must_continue_capture && game.multi_capture_row != -1 && game.multi_capture_col != -1) {
        find_captures(game.multi_capture_row, game.multi_capture_col, game.legal_moves, &game.num_legal_moves, 0, NULL);
    ...
    for (int row = 0; row < BOARD_SIZE; row++) {
        for (int col = 0; col < BOARD_SIZE; col++) {
            ...
            find_captures(row, col, all_moves, &move_count, 0, NULL);
        }
    }
...
static int lb_gen(const LBoard *lb, Move *out) {
    ...
    for (int r=0;r<BOARD_SIZE;r++) for (int c=0;c<BOARD_SIZE;c++) {
        ...
        if (cnt < MAX_LEGAL_MOVES) find_captures(r,c,out,&cnt,0,NULL);
    }
    if (cnt == 0) {
        /* Тихие ходы */
        for (int r=0;r<BOARD_SIZE;r++) for (int c=0;c<BOARD_SIZE;c++) {
